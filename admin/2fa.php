...
if (!isset($_SESSION['admin_id']) || !isset($_SESSION['2fa_pending']) || !$_SESSION['2fa_pending']) {
...
$stmt = $conn->prepare("SELECT email FROM users WHERE user_id = ? AND role = 'admin'");
...
$email_masked = preg_replace('/(?<=.).(?=.*@)/', '*', $admin['email']);

// Handle errors from resend or other actions
$error = '';
if (isset($_GET['error'])) {
    switch ($_GET['error']) {
...
// Handle success message from resend
$success = '';
...
<head>
    ...
    <title>Two-Factor Authentication - Zoryn Admin</title>
...
<body>
    <div class="verification-container">
        ...
        <form method="POST" action="verify-2fa.php" class="verification-form" autocomplete="off" id="verificationForm">
        ...
        function startTimer(duration, display) {
            ...
            const interval = setInterval(function () {
                ...
                if (--timer < 0) {
                    ...
                    window.location.href = "admin_login.php?error=2fa_expired";
                }
            }, 1000);
        }
            ...
            form.addEventListener('submit', function(e) {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        window.location.href = '2fa.php?error=' + (data.message || 'invalid_code');
                    }
                })
                .catch(error => {
                    console.error('Error:', error);
                    window.location.href = '2fa.php?error=unknown';
                });
            });
        });
    </script>
