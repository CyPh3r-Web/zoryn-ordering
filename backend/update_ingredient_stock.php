...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($type === 'add' && fifo_lots_table_exists($conn)) {
            fifo_manual_stock_in($conn, (int) $ingredient_id, $amount);
        } elseif ($type === 'remove' && fifo_lots_table_exists($conn)) {
            fifo_manual_stock_out($conn, (int) $ingredient_id, $amount);
        } else {
            // Legacy path when ingredient_lots is not migrated yet
