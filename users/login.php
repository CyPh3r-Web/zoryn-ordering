...
if (isset($_SESSION['login_attempts'])) {
    ...
    if ($login_attempts >= 3 && (time() - $last_attempt) < $cooldown_time) {
        // Remove the static error message since we show the countdown timer
        $error = '';
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && empty($error)) {
    ...
    if (empty($email) || empty($password)) {
        ...
        // Modified query to also retrieve account_status and 2FA status
        $stmt = $conn->prepare("SELECT user_id, username, password, full_name, role, account_status, two_factor_enabled FROM users WHERE email = ?");
        ...
        if ($result->num_rows === 1) {
            ...
            if ($user['account_status'] !== 'active') {
                ...
                switch ($user['account_status']) {
                    case 'pending':
                        ...
                        $error = 'Your account is not verified. <a href="verify-account.php" class="alert-link">Click here to verify your account</a>';
                        break;
                    case 'suspended':
            ...
            // If account is active, verify password
                ...
                if ($user['two_factor_enabled'] == 1) {
                    ...
                    $stmt = $conn->prepare("UPDATE users SET twofa_code = ?, twofa_expires = FROM_UNIXTIME(?), last_2fa_sent = NOW() WHERE user_id = ?");
                    $stmt->bind_param("sii", $verification_code, $expires, $user['user_id']);
                    $stmt->execute();
                    ...
                    sendVerificationEmail($user['full_name'], $email, $verification_code);
    ...
    // Check if there's an active lock when page loads
    $current_time = time();
    if (isset($_SESSION['lock_until']) && $_SESSION['lock_until'] > $current_time) {
        $isLocked = true;
        $lockTimeRemaining = $_SESSION['lock_until'] - $current_time;
        $error = "Too many login attempts. Please wait for the timer to complete.";
    } else if (isset($_SESSION['lock_until'])) {
        // Clear the lock if time has passed
...
<head>
    ...
    <style>
        ...
        .error-message a.alert-link {
            color: #721c24;
            font-weight: 700;
            text-decoration: underline;
        }
        .error-message a.alert-link:hover {
